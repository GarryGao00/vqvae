...
def reconstruct(model, x, device, dataset_type='MNIST'):
    ...
    original_img = einops.rearrange(x, '(n1 n2) c h w -> (n1 h) (n2 w) c', n1=n1)
    original_img = (original_img.clip(0, 1) * 255).cpu().numpy().astype(np.uint8)
    # original_img = torch.squeeze(original_img)

    # plt.imshow(original_img)
    # cv2.imwrite(f'work_dirs/vqvae_reconstruct_orign_{dataset_type}.jpg', original_img)
...
if __name__ == '__main__':
    ...
    print("finish 3. Train Generative model")

    # 4. Sample VQVAE
    vqvae.load_state_dict(torch.load(cfg['vqvae_path']))
    # gen_model.load_state_dict(torch.load(cfg['gen_model_path']))
    gen_model.load_state_dict(torch.load(cfg['gen_model_path'], map_location=torch.device('cpu')))
