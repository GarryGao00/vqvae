...
class VQVAE(nn.Module):
    ...
    def __init__(self, input_dim, dim, n_embedding):
        super().__init__()
        self.encoder = nn.Sequential(nn.Conv2d(input_dim, dim, 4, 2, 1),nn.ReLU(),  #subsampled
                                     nn.Conv2d(dim, dim, 4, 2, 1), nn.ReLU(),  #subsampled
        ...
        self.vq_embedding.weight.data.uniform_(-1.0 / n_embedding,
                                               1.0 / n_embedding)  #Initialize uniformly
        self.decoder = nn.Sequential(
            nn.Conv2d(dim, dim, 3, 1, 1),
            ResidualBlock(dim), ResidualBlock(dim),
            nn.ConvTranspose2d(dim, dim, 4, 2, 1), nn.ReLU(),  #upsampling
            nn.ConvTranspose2d(dim, input_dim, 4, 2, 1))  #upsampling
        self.n_downsample = 2
